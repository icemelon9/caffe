...
void LocalConvLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(bottom.size(), 1) << "Local conv Layer takes a single blob as input.";
  CHECK_EQ(top.size(), 1) << "Local conv Layer takes a single blob as output.";

  kernel_size_ = this->layer_param_.local_conv_param().kernel_size();
  stride_ = this->layer_param_.local_conv_param().stride();
  pad_ = this->layer_param_.local_conv_param().pad();
  num_ = bottom[0]->num();
...
template <typename Dtype>
void LocalConvLayer<Dtype>::Backward_cpu(
